"use client";
import Link from "next/link";

/* ─── Icons ─────────────────────────────────────────── */
const CodeIcon = () => (
  <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24">
    <polyline points="16 18 22 12 16 6" />
    <polyline points="8 6 2 12 8 18" />
  </svg>
);

const CheckIcon = () => (
  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
    <polyline points="20 6 9 17 4 12" />
  </svg>
);

/* ─── SERVICES ─────────────────────────────────────── */
const services = [
  {
    title: "Business Websites",
    desc: "High-converting, fast and SEO-friendly websites that build trust and generate leads.",
    points: ["SEO Optimized", "Mobile First", "Fast Loading"],
  },
  {
    title: "E-Commerce Development",
    desc: "Scalable online stores with secure payments and smooth checkout experience.",
    points: ["Shopify / Custom", "Payment Gateway", "Order Automation"],
  },
  {
    title: "Web Applications",
    desc: "Powerful, scalable web applications built using modern frameworks.",
    points: ["React / Next.js", "API Integration", "Cloud Ready"],
  },
];

/* ─── Component ─────────────────────────────────────── */
export default function WebDevelopmentServicePage() {
  return (
    <div className="bg-[#080c18] text-slate-100 font-sans">

      {/* ── HERO ─────────────────────────────────────── */}
      <section className="relative overflow-hidden">
        <div className="absolute inset-0 bg-blue-700/[0.05] blur-3xl
          rounded-full w-[900px] h-[500px] top-[-220px] left-1/2 -translate-x-1/2" />

        <div className="relative max-w-6xl mx-auto px-6 py-16 text-center">
          <span className="inline-block text-[11px] tracking-[3px] uppercase
            text-blue-400 bg-blue-900/20 border border-blue-700/30
            px-4 py-1.5 rounded-full mb-5">
            Website Development
          </span>

          <h1 className="text-[clamp(30px,5vw,54px)] font-extrabold leading-tight mb-4">
            Build Fast, Secure &{" "}
            <span className="bg-gradient-to-r from-blue-400 to-sky-400
              bg-clip-text text-transparent">
              High-Converting Websites
            </span>
          </h1>

          <p className="max-w-2xl mx-auto text-slate-500 text-[15.5px] leading-relaxed">
            We design and develop modern websites that load fast, rank on Google,
            and convert visitors into customers.
          </p>

          <div className="flex justify-center gap-4 mt-8 flex-wrap">
            <Link
              href="/contact-us"
              className="px-7 py-3 bg-blue-600 hover:bg-blue-500
                rounded-xl font-semibold transition"
            >
              Get a Free Quote
            </Link>

            <Link
              href="/services"
              className="px-7 py-3 border border-blue-900/40 rounded-xl
                text-slate-400 hover:text-blue-300 transition"
            >
              View Services →
            </Link>
          </div>
        </div>
      </section>

      {/* ── SERVICES ─────────────────────────────────── */}
      <section className="max-w-6xl mx-auto px-6 py-14">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {services.map((s, i) => (
            <div
              key={i}
              className="rounded-2xl p-6 bg-[#0d1424]/90 border border-blue-900/30
                hover:-translate-y-2 hover:shadow-xl hover:shadow-blue-900/25
                transition-all"
            >
              <div className="w-14 h-14 rounded-xl bg-blue-600/20 text-blue-400
                flex items-center justify-center mb-4 border border-blue-700/30">
                <CodeIcon />
              </div>

              <h3 className="text-lg font-bold mb-2">{s.title}</h3>
              <p className="text-sm text-slate-500 mb-4">{s.desc}</p>

              <ul className="space-y-2">
                {s.points.map(p => (
                  <li key={p} className="flex items-center gap-2 text-sm text-slate-400">
                    <span className="w-5 h-5 flex items-center justify-center rounded-full
                      bg-blue-900/30 border border-blue-700/30 text-blue-400">
                      <CheckIcon />
                    </span>
                    {p}
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>
      </section>

      {/* ── WHY CHOOSE US ───────────────────────────── */}
      <section className="bg-blue-900/[0.04] border-y border-blue-900/20">
        <div className="max-w-6xl mx-auto px-6 py-14 grid md:grid-cols-2 gap-10">
          <div>
            <h2 className="text-3xl font-extrabold mb-4">
              Why Choose Our Web Development Services?
            </h2>
            <p className="text-slate-500 mb-5">
              We don’t just build websites — we build digital assets that grow your business.
            </p>

            <ul className="space-y-3">
              {[
                "Lightning-fast performance",
                "SEO-friendly architecture",
                "Mobile-first responsive design",
                "Secure & scalable codebase",
              ].map(item => (
                <li key={item} className="flex items-center gap-3 text-slate-400">
                  <span className="w-5 h-5 rounded-full bg-blue-600/20
                    text-blue-400 flex items-center justify-center">
                    <CheckIcon />
                  </span>
                  {item}
                </li>
              ))}
            </ul>
          </div>

          <div className="grid grid-cols-2 gap-4">
            {["Next.js", "React", "Tailwind CSS", "Node.js"].map(t => (
              <div
                key={t}
                className="rounded-xl bg-[#0d1424]/90 border border-blue-900/30
                  p-5 text-center text-blue-400 font-semibold"
              >
                {t}
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* ── CTA ─────────────────────────────────────── */}
      <section className="max-w-6xl mx-auto px-6 py-16 text-center">
        <h2 className="text-[clamp(28px,5vw,44px)] font-extrabold mb-4">
          Ready to Build Your Website?
        </h2>
        <p className="text-slate-500 max-w-xl mx-auto mb-6">
          Let’s create a high-performance website that drives leads and sales.
        </p>

        <Link
          href="/contact-us"
          className="inline-block px-9 py-3.5 bg-blue-600 hover:bg-blue-500
            rounded-xl font-semibold text-lg transition
            hover:shadow-lg hover:shadow-blue-700/40"
        >
          Book Free Consultation
        </Link>
      </section>

    </div>
  );
}
